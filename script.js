...
function sortAdsTable(colIdx, type) {
    ...
    table.querySelectorAll('th').forEach(h => { h.dataset.sort = ''; const ic = h.querySelector('.sort-icon'); if(ic) ic.textContent='⇅'; });
    th.dataset.sort = asc ? 'asc' : 'desc';
    const ic = th.querySelector('.sort-icon'); if(ic) ic.textContent = asc ? '↑' : '↓';
    rows.sort((a, b) => {
        const av = a.cells[colIdx]?.dataset?.value ?? a.cells[colIdx]?.textContent ?? '';
        const bv = b.cells[colIdx]?.dataset?.value ?? b.cells[colIdx]?.textContent ?? '';
